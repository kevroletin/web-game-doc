...
#####Description:
**sid** must be a session id of the current player, otherwise {"result": "badJson"}
**regions** must be a list of regions that belong to this user and have his current race. The sum of tokensNum must be **equal** to the current total number of tokens, otherwise {"result": "badTokensNum"}. If user has no tokens, it returns  {"result": "userHasNotRegions"}. 
**encampments** field can only be executed by user with Bivouacking special power. It consists of the list of regions that belong this user with this special power. Sum of encampmentsNum must not be greater than 5, otherwise it returns  {"result": "notEnoughEncampentsForRedeployment"}. 
**fortifield** field can be executed only by user with special power Fortifield. It consists of only one field -- fortifield, where user wants to set up the fortress. It should be the region of this user and this race, otherwise -- {"result": "badRegion"}. A region can't have more than one fortress, otherwise -- {"result": "tooManyFortifieldsInRegion"}. User cannot set more fortresses than current maximum number, otherwise -- {"result": "tooManyFortifields"}
**heroes** allows user with special power Heroic to set two heroes on his regions. If the number of heroes greater than 2, it returns {"result": "badSetHeroCommand"}. 
**selectFriend** allows user with special power Diplomat choose the friend. friendId must be id of the user playing the same game, otherwise {"result": "badFriendId"}. It must be the id of user that wasn't attacked by current user on this turn, otherwise --  {"result": "badFriend"}.
This command can only be executed after following commands: "conquer", "throwDice", "defend"
